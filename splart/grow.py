...
from splart.splat_components import in_place_pack_into_splat_tensor_starting_from
...
def get_brush_scale_for_l1_loss(config: TrainingConfig, current_l1_loss: float, current_epoch: int) -> float:
    ...
    n_pixels = np.prod(config.target_image_load_size.as_w_h())
    ...
    image_min_extend = min(config.target_image_load_size.as_w_h())
    ...
    scale = desired_brush_size / config.texture_load_size.width  # width or height does not matter here, they are equal
...
def in_place_add_samples_starting_from(
    ...
    target_w_h = config.target_image_load_size.as_w_h()  # w, h
    new_positions_absolute_xy = np.random.randint(0, target_w_h, size=(n_samples, 2))  # x, y
    new_positions_unit_normalized = torch.tensor(new_positions_absolute_xy / target_w_h, device=config.device).float()
    new_positions_ndc = 2 * new_positions_unit_normalized - 1  # map to [-1, 1]

    # Fetching the colour of the pixels at each coordinate pair
    # numpy expects y,x!
    new_colors_np = np.array([target_image_np[y, x] for x, y in new_positions_absolute_xy])
